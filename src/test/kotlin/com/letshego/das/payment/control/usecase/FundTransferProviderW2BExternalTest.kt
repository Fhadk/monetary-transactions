...
class FundTransferProviderW2BExternalTest {
    ...
    @InjectMocks
    private lateinit var fundTransferProviderW2BExternal: FundTransferProviderW2BExternal

    @BeforeEach
    fun setUp() {
        ReflectionTestUtils.setField(fundTransferProviderW2BExternal, "paymentStatus", publishPaymentStatus)
    }
    ...
    fun pay() {
        ...
        Mockito.doReturn(mobifinRes).`when`(paymentClient).initiateWalletToBankExternalTransfer(any(), any())
        val result = fundTransferProviderW2BExternal.pay(req,"WK1234567890")
    ...
    @Test
    fun `pay for Exception for ghana`() {
        val req = TestUtil.paymentRequestDto()
        val mobifinRes = TestUtil.getMobifinResponseForFailure()
        val map = HashMap<CountryCodes, CountryProperties.CountryConfig>()
        val config = CountryProperties.CountryConfig()
